...
setup_sqlite_production_db() {
  ...
  curl "$CONFIG_WEB_TYPO3_PUBLIC/index_dev.php/typo3cr/setup/setup?dsn=sqlite:$CONFIG_PRODUCTION_DB_PATH&indexlocation=$CONFIG_LUCENE_INDEX_LOCATION" | sed -e 's/<[^>]*>//g'
  echo
  enable_persistence
...
setup_postgres_production_db() {
  ...
  disabled_persistence
  curl "$CONFIG_WEB_TYPO3_PUBLIC/index_dev.php/typo3cr/setup/setup?dsn=pgsql:dbname=$CONFIG_PRODUCTION_DB_NAME&userid=$CONFIG_PRODUCTION_DB_USER&password=$CONFIG_PRODUCTION_DB_PASS&indexlocation=$CONFIG_LUCENE_INDEX_LOCATION" | sed -e 's/<[^>]*>//g'
  echo
  enable_persistence
  echo "Writing Configuration/Settings.php..."
...
setup_mysql_production_db() {
  ...
  disabled_persistence
  curl "$CONFIG_WEB_TYPO3_PUBLIC/index_dev.php/typo3cr/setup/setup?dsn=mysql:dbname=$CONFIG_PRODUCTION_DB_NAME&userid=$CONFIG_PRODUCTION_DB_USER&password=$CONFIG_PRODUCTION_DB_PASS&indexlocation=$CONFIG_LUCENE_INDEX_LOCATION" | sed -e 's/<[^>]*>//g'
  echo
  enable_persistence
  echo "Writing Configuration/Settings.php..."
...
setup_typo3() {
  ...
  echo "Fixing permissions of lucene index location..."
  chown -R :$CONFIG_WEBSERVER_GROUP "$CONFIG_LUCENE_INDEX_LOCATION"
  chmod 775 "$CONFIG_LUCENE_INDEX_LOCATION"
